...
def generate_cloud_texture(shape=(100, 100), seed=42, intensity=1.0, roughness=5.0):
    ...
    # 2. Apply Gaussian Blur (The key to cloud realism)
    # Higher sigma = smoother clouds
...
def scan_single_sector(sector_name):
    ...
    conditions = [
        ...
        # High Ice Cloud (Cirrus - Unseedable)
    ...
    data['prob'] = max(0.0, min(100.0, data['prob']))  # FORCE 0-100 range
...
def plot_scientific_matrix(data_points):
    ...
    # We use generate_cloud_texture() for anything spatial
    ...
    plots = [
        ...
        {"ax": axes[1,3], "title": "Vertical Velocity (m/s)", "cmap": "RdBu_r", "data": (generate_cloud_texture(seed=seed+8, roughness=3) - 0.5) * 10, "vmax": 5}, # Updraft/downdraft
    ...
    for p in plots:
        ...
        im = ax.imshow(p['data'], cmap=p['cmap'], aspect='auto') #, vmin=0, vmax=p.get('vmax', None))
...
if 'all_sector_data' not in st.session_state:
    ...
    # Sort regions by probability initially to select the best one
...
with st.sidebar:
    ...
    st.title("VisionRain")
    st.caption("Kingdom Commander | v22.4")
    
    st.write("---")
    
    # REGION SELECTOR (Updates the Main View)
    st.markdown("### 📡 Active Sector")
    
    # Create list, ensuring current selected is valid
...
tab1, tab2, tab3 = st.tabs(["🌍 Strategic Pitch", "🛰️ Operations & Surveillance", "🧠 Gemini Autopilot"])
...
with tab2:
    ...
    with col_map:
        ...
        for region_name, info in SAUDI_SECTORS.items():
            ...
            tooltip_html = f"""
            <b>{region_name}</b><br>
            Prob: {r_data['prob']:.1f}%<br>
            Rad: {r_data['rad']:.1f}µm<br>
            Temp: {r_data['temp']:.1f}C
            """
            
            folium.Marker(
                info['coords'],
                popup=tooltip_html,
                tooltip=f"{region_name}: {r_data['prob']:.0f}%",
                icon=folium.Icon(color=color, icon="cloud", prefix="fa")
            ).add_to(m)
            
            # Highlight Circle for CURRENT selection
            if region_name == current_region:
                folium.CircleMarker(
                    info['coords'], radius=20, color="#00e5ff", fill=True, fill_opacity=0.2
                ).add_to(m)
    ...
    for reg, d in st.session_state.all_sector_data.items():
        table_data.append({
            ...
            "Probability": d['prob'], # Keep raw for progress bar
    ...
    st.dataframe(
        ...
        column_config={
            "Probability": st.column_config.ProgressColumn(
                "Probability",
                help="Cloud Presence Probability (0-100%)",
                format="%.1f%%",
                min_value=0,
                max_value=100,
            ),
...
# TAB 3: GEMINI
with tab3:
    st.header(f"AI Commander: {current_region}")
    ...
    if st.button("🚀 REQUEST AUTHORIZATION", type="primary"):
        with st.status("Analyzing Physics...") as status:
            prompt = f"""
            ...
            # Logic if No API Key
            if not api_key:
                st.warning("⚠️ Offline Mode (Simulated AI)")
                if current_data['prob'] > 60 and current_data['rad'] < 14 and current_data['phase'] == 1:
                    decision = "GO"
                    response_text = "Conditions Optimal. Liquid water present with harvestable droplet size."
                else:
                    decision = "NO-GO"
                    response_text = f"Conditions Unfavorable. Radius {current_data['rad']:.1f}µm or Phase unsuitable."
            else:
                try:
