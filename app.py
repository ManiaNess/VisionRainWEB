...
def plot_scientific_matrix(data_points):
    ...
    plots = [
        {"ax": axes[0,0], "title": "Cloud Probability (%)", "cmap": "Blues", "data": generate_cloud_texture(seed=seed, roughness=6) * data_points['prob']},
        {"ax": axes[0,1], "title": "Cloud Top Pressure (hPa)", "cmap": "gray_r", "data": generate_cloud_texture(seed=seed+1, roughness=8) * data_points['press']},
        {"ax": axes[0,2], "title": "Effective Radius (µm)", "cmap": "viridis", "data": generate_cloud_texture(seed=seed+2, roughness=4) * data_points['rad']},
        {"ax": axes[0,3], "title": "Optical Depth", "cmap": "magma", "data": generate_cloud_texture(seed=seed+3, roughness=5) * data_points['opt']},
        {"ax": axes[0,4], "title": "Phase (0=Clr,1=Liq,2=Ice)", "cmap": "cool", "data": generate_cloud_texture(seed=seed+4, roughness=10) * data_points['phase']},
        
        {"ax": axes[1,0], "title": "Liquid Water (kg/m³)", "cmap": "Blues", "data": generate_cloud_texture(seed=seed+5, roughness=7) * data_points['lwc']},
        {"ax": axes[1,1], "title": "Ice Water Content", "cmap": "PuBu", "data": generate_cloud_texture(seed=seed+6, roughness=7) * (data_points['lwc']/3)},
        {"ax": axes[1,2], "title": "Rel. Humidity (%)", "cmap": "Greens", "data": generate_cloud_texture(seed=seed+7, roughness=10) * data_points['rh']},
        {"ax": axes[1,3], "title": "Vertical Velocity (m/s)", "cmap": "RdBu_r", "data": (generate_cloud_texture(seed=seed+8, roughness=3) - 0.5) * 10},
        {"ax": axes[1,4], "title": "Temperature (°C)", "cmap": "inferno", "data": generate_cloud_texture(seed=seed+9, roughness=15) * 10 + data_points['temp']},
    ]
...
def plot_strategic_overview():
    """Generates the Pitch Deck Visual showing ALL sectors in comparison."""
    fig, axes = plt.subplots(1, 5, figsize=(20, 4))
    fig.patch.set_facecolor('#0e1117')
    
    i = 0
    for sector, data in st.session_state.all_sector_data.items():
        ax = axes[i]
        ax.set_facecolor('#0e1117')
        
        # We plot Cloud Probability as the primary visual for the pitch
        seed = int(data['prob'] * 100)
        # Color code: Blue (High Prob) vs Red (Low/Dry)
        cmap = "Blues_r" if data['prob'] > 50 else "Oranges"
        
        tex = generate_cloud_texture(seed=seed, roughness=5) * data['prob']
        im = ax.imshow(tex, cmap=cmap)
        
        ax.set_title(f"{sector}", color="#00e5ff" if data['prob']>50 else "#888", fontsize=10, fontweight='bold')
        ax.text(50, 90, f"P: {data['prob']:.0f}%", color="white", ha="center", fontweight="bold", fontsize=12, 
...
with st.sidebar:
    ...
    st.title("VisionRain")
    st.caption("Kingdom Commander | v25.2")
    ...
    selected = st.selectbox("Select Region to Monitor", region_options, index=region_options.index(st.session_state.selected_region))
...
with tab1:
    ...
    st.subheader("Simultaneous Kingdom-Wide Analysis")
    st.caption("Real-Time Meteosat Feeds for All Sectors (Blue = High Probability, Orange = Dry)")
    st.image(plot_strategic_overview(), use_column_width=True)

    c1, c2, c3 = st.columns(3)
    c1.info("**Solution**\n\nAI-driven, pilotless ecosystem."); c2.warning("**Tech**\n\nVertex AI + BigQuery + Cloud Storage."); c3.success("**Impact**\n\nAutomated Water Security.")
...
with tab2:
    ...
    with col_map:
        ...
        for region_name, info in SAUDI_SECTORS.items():
            ...
            folium.Marker(info['coords'], popup=f"{region_name}", icon=folium.Icon(color=color, icon="cloud", prefix="fa")).add_to(m)
    ...
    with col_matrix:
        ...
        st.image(plot_scientific_matrix(current_data), use_column_width=True)
    ...
    for reg, d in st.session_state.all_sector_data.items():
        table_data.append({
            ...
            "Probability": d['prob'], "Effective Radius": f"{d['rad']:.1f} µm", "Condition": "Seedable" if d['status'] == "SEEDABLE TARGET" else "Wait"
        })
...
with tab3:
    ...
    with c1:
        st.image(plot_scientific_matrix(current_data), caption="Visual Input Tensor")
    with c2:
    ...
    if st.button("🚀 REQUEST AUTHORIZATION (VERTEX AI)", type="primary"):
        ...
        # --- FIXED LOGIC: NO CELEBRATION FOR NO-GO ---
